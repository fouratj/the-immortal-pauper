...
When unscheduled bulk transfers hit Myanmar through an irregular routing path, the formal systems processed them as expected. But the informal networks—the ones that operated on social trust rather than blockchain verification—saw the same transfers and raised alarms through channels the cascade couldn't see. Community banking operators calling each other. Local cooperative leaders flagging anomalies to regulators they knew personally. The trust architectures that predated the II by decades, doing what they'd always done: protecting their members from things that looked wrong.

Myanmar's fraud filters hadn't just flagged the volume and routing path. They'd been primed by a wave of informal reports from networks the cascade didn't know existed. By the time Damon found the 0.3%, the gap between what ARBITER could model and what actually mattered had become fourteen million people in legal limbo.
...
"사람도 그래." *So do people.* A pause. "서진아. 무슨 일이야? 목소리가 달라."
